    ...
    addTaskForm.onsubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                addTaskForm.reset();
                modal.style.display = 'none';
                fetchTasks(); // Refresh tasks instead of page reload
            }
        } catch (error) {
...
function createTaskRow(task) {
    ...
    row.innerHTML = `
        <td>${sanitizeHTML(task.title)}</td>
        <td>${dueDate}</td>
        <td><span class="priority priority-${task.priority.toLowerCase()}">${sanitizeHTML(task.priority)}</span></td>
    `;
    row.id = `task-${task.id || task.task_id}`;
    row.classList.add('clickable_row');
    row.addEventListener('click', () => showDetails(task.task_id));
...
function handleContentEditableChange(event, taskId) {
    if (event.key === 'Enter' && !event.shiftKey) {
        ...
        element.blur();
        saveToDatabase(taskId, field, value)
            .then(() => {
...
async function saveToDatabase(taskId, field, value) {
    try {
...
function makeFieldsEditable(taskId) {
    ...
    editableFields.forEach(field => {
        ...
        field._keydownHandler = (e) => handleContentEditableChange(e, taskId);
...
// Replace existing showToast function with new implementation
